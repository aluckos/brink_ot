...
inline void BrinkOpenTherm::setup() {
  ...
  ESP_LOGD("brink", "BrinkOpenTherm setup complete");
}
...
inline void BrinkOpenTherm::update() {
  ...
  switch(current_step) {
    case 0: // Nastawa mocy (ID 71)
      ...
      ESP_LOGD("brink", "Step 0: Ustawianie mocy %.1f%%", target_ventilation);
      current_step++; break;
    ...
    case 1: // T1 Czerpnia (ID 80)
      if (t_supply_in_sensor) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 1: T1 Czerpnia = %.2f°C", temp);
        } else {
          ESP_LOGW("brink", "Step 1: Brak odpowiedzi dla T1 (response=0x%08lX)", response);
        }
      }
      current_step++; break;

    case 2: // T2 Nawiew do domu (ID 81) [NOWY]
      if (t_supply_out_sensor) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 2: T2 Nawiew = %.2f°C", temp);
        } else {
          ESP_LOGW("brink", "Step 2: Brak odpowiedzi dla T2 (response=0x%08lX)", response);
        }
      }
      current_step++; break;
    ...
    case 3: // T3 Wywiew z domu (ID 82)
      if (t_exhaust_in_sensor) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 3: T3 Wywiew = %.2f°C", temp);
        } else {
          ESP_LOGW("brink", "Step 3: Brak odpowiedzi dla T3 (response=0x%08lX)", response);
        }
      }
      current_step++; break;

    case 4: // T4 Wyrzutnia na zewnątrz (ID 83) [NOWY]
      if (t_exhaust_out_sensor) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 4: T4 Wyrzutnia = %.2f°C", temp);
        } else {
          ESP_LOGW("brink", "Step 4: Brak odpowiedzi dla T4 (response=0x%08lX)", response);
        }
      }
      current_step++; break;
    ...
    case 5: // Przepływ m3/h (TSP 52 - Low Byte)
      if (current_flow_sensor) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 5: Przepływ Low Byte = 0x%02X", temp_lb);
        } else {
          ESP_LOGW("brink", "Step 5: Brak odpowiedzi dla przepływu Low Byte");
        }
      }
      current_step++; break;
    ...
    case 6: // Przepływ m3/h (TSP 53 - High Byte)
      if (current_flow_sensor) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 6: Przepływ = %u m³/h", flow);
        } else {
          ESP_LOGW("brink", "Step 6: Brak odpowiedzi dla przepływu High Byte");
        }
      }
      current_step++; break;
    ...
    case 7: // Status Filtra (TSP 13)
      if (filter_status_binary) {
        ...
        if (response) {
          ...
          ESP_LOGD("brink", "Step 7: Status filtra = %s", filter_dirty ? "brudny" : "czysty");
        } else {
          ESP_LOGW("brink", "Step 7: Brak odpowiedzi dla statusu filtra");
        }
      }

...
// Klasa do sterowania suwakiem (wentylacja %)
...
class BrinkOpenTherm : public PollingComponent {
 public:
  ...
  // Sensory numeryczne (T1-T4 + Przepływ)
  ...
  // Sensor binarny (Stan filtra)
  binary_sensor::BinarySensor *filter_status_binary{nullptr};

  // Sensory tekstowe (Status i Biegi)
  ...
  // Funkcje łączące kod C++ z plikami .py i YAML
...
inline void BrinkOpenTherm::update() {
  ...
  // --- MASZYNA STANÓW: Odczyt parametrów krok po kroku ---
  switch(current_step) {
    case 0: // Wysłanie nastawy wentylacji
      ot->sendRequest(ot->buildRequest(OpenThermMessageType::WRITE_DATA, (OpenThermMessageID)71, (unsigned int)target_ventilation));
      current_step++; break;

    case 1: // T1 - Czerpnia (Standard OT ID 80)
    ...
    case 2: // T2 - Nawiew (z rejestru TSP 41)
